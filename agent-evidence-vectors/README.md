...
[agent-evidence-vectors](https://github.com/astrogilda/agent-evidence-vectors) is a conformance
vector suite and reference verifier for evidence about what an automated agent did at run time.
The eight corpus manifests at release v0.10.1 sum to 461 vectors, judged by one Go verifier,
`aee-verify`, selected by the suite each corpus manifest declares.

## How it uses in-toto

Every vector is an in-toto Statement, and two of the eight corpora exercise proposed predicates
directly:
...
and returned 61/61 accepts, 209/209 rejects and 2/2 indeterminate with zero mismatches. That row
